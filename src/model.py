...
class CausalSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        self.register_buffer("mask", torch.tril(torch.ones(config.block_size, config.block_size))
                             .view(1,1, config.block_size, config.block_size)) # Taking the lower triangular -> setting it to Zero
    ...
    def forward(self, X, layer_past=None):
        ...
        att = att.masked_fill(self.mask[:,:,:T,:T] == 0, float('-inf')) # If lower triangular (future tokens), make it impossible 
...
class Block(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(config.n_emb, 4 * config.n_emb), # Bottle-Neck Layer
            nn.GELU(),
...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.pos_emb = nn.Parameter(torch.zeros(1, config.block_size, config.n_emb))
