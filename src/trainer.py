...
class LayoutTransformerTrainer:
    ...
    def train(self):
        ...
        if self.config.ckpt_path is not None:
            ...
            metric_history['loss'] = metric_history['loss'].tensor.detach().numpy()
            metric_history['val_loss'] = metric_history['loss'].tensor.detach().numpy()
        ...
        for epoch in range(start_epoch+1, self.config.epoch+1):
            ...
            # Metrics
            metric_history['loss'].append(train_loss.tensor.detach().numpy())
            metric_history['val_loss'].append(val_loss.tensor.detach().numpy())
            # Log messages
            logs = {}
